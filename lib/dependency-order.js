...
const debug = require('debug')('things-factory:env:dependency-order')
...
try {
  ...
  /* shell 모듈을 맨 말단으로 하기위해서 shell은 포함시키지 않는다. */
  ...
  if (orderedModuleNames.length == 0) {
    /* 
      orderedModuleNames가 비어있는 경우는, 현재 모듈이 '@things-factory/shell'인 경우이다.
     */
    orderedModuleNames.push(pkg.name)
  }

  debug(
    'ORDERED MODULE LIST\n',
    orderedModuleNames.map((m, idx) => `${idx} : ${m}`)
  )
